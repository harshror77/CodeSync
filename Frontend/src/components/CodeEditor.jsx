...
const CodeEditor = () => {
    ...
    useEffect(() => {
        ...
        const execSocket = io(`${socketUrl}/code-execution`, { transports: ['websocket'] });
        ...
        socket.on('room-joined', (data) => setRoomUsers(data.users || []));
        socket.on('user-connected', (user) => setRoomUsers(prev => prev.some(u => u.userId === user.userId) ? prev : [...prev, user]));
        socket.on('user-left', (data) => setRoomUsers(prev => prev.filter(u => u.userId !== data.userId)));
        ...
        <div className="flex flex-col h-screen bg-[#09090b] text-zinc-300 font-sans overflow-hidden">
            <header className="h-14 border-b border-zinc-800 flex items-center justify-between px-6 bg-[#09090b] shrink-0">
                <div className="flex items-center gap-4">
                    <span className="text-white font-bold text-lg">CodeHaven <span className="text-blue-500">IDE</span></span>
                    <button onClick={() => { navigator.clipboard.writeText(roomId); setStatus(s => ({ ...s, copied: true })); setTimeout(() => setStatus(s => ({ ...s, copied: false })), 2000); }}
                        className="flex items-center gap-2 text-xs bg-zinc-900 px-3 py-1.5 rounded-md border border-zinc-800">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={onRun} disabled={status.executing} className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-1.5 rounded-md text-sm font-medium transition disabled:opacity-50">
            ...
            <main className="flex-1 flex overflow-hidden">
                <div className="w-96 border-r border-zinc-800 flex flex-col bg-[#09090b] shrink-0">
                    <div className="flex-1 overflow-y-auto"><FileExplorer roomId={roomId} onFileSelect={handleFileSelect} /></div>
                    <div className="p-4 border-t border-zinc-800 bg-[#0c0c0e]">
                        <h3 className="text-[10px] uppercase tracking-widest font-bold text-zinc-500 flex items-center gap-2 mb-3"><Users size={12} /> Active Users ({roomUsers.length})</h3>
                ...
                <div className="flex-1 flex flex-col bg-[#0c0c0e] min-w-0">
                    {currentFile ? (
                        <>
                            <div className="flex-1 relative"><div className="absolute inset-0" ref={editorRef} /></div>
                            <div className="h-56 border-t border-zinc-800 bg-[#09090b] flex flex-col shrink-0">
                                <div className="flex items-center justify-between px-4 py-2 border-b border-zinc-800 bg-zinc-900/50">
                                    ...
                                    <button onClick={() => setTerminal([])} className="text-[10px] hover:text-white">Clear</button>
                                </div>
                                <div className="flex-1 overflow-y-auto p-4 font-mono text-xs space-y-1">
                                    {terminal.map((t, i) => (
                                        <div key={i} className="flex gap-2">
                                            <span className="text-zinc-600">[{t.time}]</span>
                                            <span className={t.type === 'error' ? 'text-red-400' : t.type === 'output' ? 'text-green-400' : 'text-blue-400'}>{t.msg}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
